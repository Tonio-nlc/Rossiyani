# Rossiyani Documentation

## 1. Lire TOUJOURS en premier

product/north_star.md
product/product_constitution.md
product/ui_principles.md

Ces documents sont prioritaires sur tous les autres.

---

## 2. Comprendre le produit

product/glossary.md
product/roadmap.md
product/flows/

---

## 3. Comprendre la technique

ARCHITECTURE.md
MEDIA_LAYER.md
SPECIFICATION.md
UI_SPEC.md

---

## 4. Comprendre le contenu

LINGUISTIC_LIBRARY.md
KNOWLEDGE_GRAPH.md
KNOWLEDGE_PIPELINE.md
MANUAL_EDITORIAL_RULES.md
CONTENT_IMPORT_SPEC.md

---

## 5. Développement

DEVELOPMENT_RULES.md
AI_ANALYSIS_SPEC.md